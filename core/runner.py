...
from position_manager import build_orders
...
if __name__ == "__main__":
    ...
    # ตัวอย่างสรุป 10 แท่งท้าย
    tail = pd.concat([df[["time","close"]].tail(10).reset_index(drop=True),
                      sig.tail(10).reset_index(drop=True),
                      plan.tail(10).reset_index(drop=True)], axis=1)
    print(tail.to_string(index=False))
    print("[ok] pipeline ready. ต่อ cTrader API ได้เมื่อพร้อม")
